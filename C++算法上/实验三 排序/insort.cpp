...
template <typename E, typename Comp>
void inssortcount(E A[], int n, int a[]) {
	int swapcount = 0;
	int compcount = 0;
	for (int i = 1; i < n; i++) {
		for (int j = i; compcount++, (j > 0) && (Comp::prior(A[j], A[j - 1])); j--) {
			swap(A, j, j - 1);
			swapcount++;
		}
	}
...
void inssort(int n) {
	...
	QueryPerformanceCounter(&begin_time);
	inssort<int, minintCompare>(worstArraydata, n);
	QueryPerformanceCounter(&end_time);
	ns_time = (end_time.QuadPart - begin_time.QuadPart) * 1000.0 / freq_.QuadPart;
	cout << "time use=" << ns_time << "ms" << endl;
	inssortcount<int, minintCompare>(worstArraydatacount, n, a);
	cout << "swapcount: " << a[0] << endl;
	cout << "compcount: " << a[1] << endl;
	a[0] = 0; //clear
	a[1] = 0;
	...
	for (int j = 0; j < number; j++)
	{
		...
		QueryPerformanceCounter(&begin_time);
		inssort<int, minintCompare>(averArraydata, n);
		QueryPerformanceCounter(&end_time);
		ns_time = (end_time.QuadPart - begin_time.QuadPart) * 1000.0 / freq_.QuadPart;
		sum_time = sum_time+ns_time;
		inssortcount<int, minintCompare>(averArraydatacount, n, a);
