...
def read_dataset(filename):
    ...
    for line in all_lines[0:]:
        ...
        dataset.append(line)
    # for i in range(len(dataset)):       # str数据转float型
...
def read_testset(testfile):
    ...
    for line in all_lines[0:]:
        line = line.strip().split(',')  # 以逗号为分割符拆分列表
        testset.append(line)
    # for i in range(len(testset)):       # str数据转float型
...
def C45_createTree(dataset, labels, test_dataset):
    ...
    if post_pruning:
        tree_output = classifytest(C45Tree, featLabels=labels, testDataSet=test_dataset)
        ans = []
        for vec in test_dataset:
            ans.append(vec[-1])
        cut_acc = cal_acc(tree_output, ans)
...
def CART_createTree(dataset, labels, test_dataset):
    ...
    if post_pruning:
        tree_output = classifytest(CARTTree, featLabels=labels, testDataSet=test_dataset)
        ans = []
        for vec in test_dataset:
            ans.append(vec[-1])
        cut_acc = cal_acc(tree_output, ans)
...
if __name__ == '__main__':
    ...
    # C4.5决策树
    if dec_tree == '2':
        C45decisionTree = C45_createTree(dataset, labels, test_dataset=read_testset(testfile))
        print('C45decisionTree:\n', C45decisionTree)
        DecisionTreePlotter.C45_Tree(C45decisionTree)
        testSet = read_testset(testfile)
        print("下面为测试数据集结果：")
        print('C4.5_TestSet_classifyResult:\n', classifytest(C45decisionTree, labels, testSet))
        print("---------------------------------------------")

    # CART决策树
    if dec_tree == '3':
        CARTdecisionTree = CART_createTree(dataset, labels, test_dataset=read_testset(testfile))
        print('CARTdecisionTree:\n', CARTdecisionTree)
        DecisionTreePlotter.CART_Tree(CARTdecisionTree)
        testSet = read_testset(testfile)
        print("下面为测试数据集结果：")
        print('CART_TestSet_classifyResult:\n', classifytest(CARTdecisionTree, labels, testSet))
